...
# joints actuators and the tip sites of the robot hand
...
# Name of the camera used for observation/rendering that should get pose
# randomization each reset. Rename this to match scene.xml if different --
...
def _quat_angle(q):
    """Angle (rad, in [0, pi]) of the rotation represented by unit quaternion q."""
    w = float(np.clip(abs(q[0]), 0.0, 1.0))
    return 2.0 * np.arccos(w)


# Quaternion (not raw normal-vector cross products) that takes each face's
# world-frame normal to +Z, precomputed once via the same robust solver
# reset_model already used inline every reset. Reused for theta/axis math
# below so the antipodal (theta ~ pi) case is handled by _q_from_vecs's
# proper fallback instead of an ad hoc arbitrary-axis special case.
FACE_QUATS = np.array([_q_from_vecs(FACE_NORMALS[f], np.array([0.0, 0.0, 1.0]))
                        for f in range(6)])
...
class AmazeDexCubeEnv(MujocoEnv):
    ...
    def _face_error_quat(self):
        """Quaternion rotating the target face's current world-frame normal
        to world +Z (shortest arc). Its angle is the face misalignment
        theta; its vector part gives the needed rotation axis. Replaces
        separate dot-product (theta) and cross-product (axis) computations
        so both come from one robust source."""
        n = self.data.xmat[self.cubeid].reshape(3, 3) @ FACE_NORMALS[self.target_face]
        return _q_from_vecs(n, np.array([0.0, 0.0, 1.0]))

    def _theta(self):
        return _quat_angle(self._face_error_quat())
    ...
    def _desired_rot_axis_from(self, R):
        ...
        if norm < 1e-6:
            # theta ~ 0 (already aligned) -- _q_from_vecs's own antipodal
            # fallback means theta ~ pi always yields a valid nonzero axis
            # here, so this branch only fires on true alignment, not on the
    ...
    def _push_alignment(self, axis):
        ...
        for i in range(self.data.ncon):
            ...
            for t, geoms in enumerate(self.tip_geoms):
                ...
                f_world = f6[0] * frame[0] + f6[1] * frame[1] + f6[2] * frame[2]
                
                if con.geom1 in self.cube_geoms:
                    f_world = -f_world

                if np.linalg.norm(f_world) <= c_force:
                    continue
    ...
    def _reward(self, act, dropped, step_delta):
        c = self.cfg
        ...
        # Penalize actual actuator rate-of-change (post low-pass-filter,
        # post rate-limit), normalized to the max allowed step so the value
        # is in [0, 1] per joint before squaring. Previously this penalized
        # the *raw* pre-filter action delta, which has no fixed relationship
        # to physical torque/motion since the LPF + rate limiter already
        # smooth whatever the policy outputs -- that made the penalty noisy
        # and could suppress exploration without actually buying smoother
        # motion.
        r_action_rate = -c.k_action_rate * float(
            np.mean(np.square(step_delta / c.max_ctrl_rate_frac)))
        ...
        if axis is not None and theta > c.success_theta_rad:
            # Only reward spin-toward-target while there's still real distance
            # to cover -- once inside the success zone, spinning further no
            # longer helps and previously was rewarded anyway, encouraging
            # fling-through-and-past-target behavior.
    ...
    def step(self, action):
        ...
        reward, info = self._reward(act, dropped, step_delta)
